                                 ...
                                 int &xflag, int &yflag, int &zflag)
{
  ...
  for (int i = 0; i < nfield; i++) {
    ...
    // UCG addition
    else if (fieldtype[i] == UCGSTATE)
      fieldindex[i] = find_label("ucgstate", labels);
    else if (fieldtype[i] == UCGL)
      fieldindex[i] = find_label("ucgl", labels);
    else if (fieldtype[i] == UCGP)
      fieldindex[i] = find_label("ucgp", labels);
    else if (fieldtype[i] == MSUCGL)
      fieldindex[i] = find_label("msucgl", labels);
    else if (fieldtype[i] == MSUCGP)
      fieldindex[i] = find_label("msucgp", labels);
  }
