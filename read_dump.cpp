...
void ReadDump::process_atoms()
{
  ...
  double **msucgl = atom->msucgl;
  double **msucgp = atom->msucgp;
    ...
    if (replaceflag) {
      ...
      for (ifield = 1; ifield < nfield; ifield++) {
        ...
        case Reader::FZ:
          ...
          break;
        // Add UCG here
        case Reader::UCGSTATE:
          ucgstate[m] = static_cast<int> (fields[i][ifield]);
          break;
        case Reader::UCGP:
          ucgp[m] = fields[i][ifield];
          break;
        case Reader::UCGL:
          ucgl[m] = fields[i][ifield];
          break;
        // Multisite UCG, actually need to enumerate EVERY substate, like MSUCGLA, MSUCGLB, MSUCGLC, etc.
        // Placeholder for now! To do in the future.
        }
      }
  ...
  // NOTE: UCG currently does not support this, will do later
  // ----------------------------------------------------
  ...
  for (i = 0; i < nnew; i++) {
    ...
    for (ifield = 0; ifield < nfield; ifield++) {
      ...
      case Reader::UCGL:
        ...
        break;
      // Multisite UCG, actually need to enumerate EVERY substate, like MSUCGLA, MSUCGLB, MSUCGLC, etc.
      // Placeholder for now! Will do later.
      }
...
int ReadDump::fields_and_keywords(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    ...
    // UCG additions
    if (type == Reader::UCGSTATE && !atom->ucg_flag)
      error->all(FLERR,"Read dump of UCG state property that isn't supported by atom style");
    if (type == Reader::UCGL && !atom->ucg_flag)
      error->all(FLERR,"Read dump of UCG L property that isn't supported by atom style");
    if (type == Reader::UCGP && !atom->ucg_flag)
      error->all(FLERR,"Read dump of UCG P property that isn't supported by atom style");
    if (type == Reader::MSUCGL && !atom->msucg_flag)
      error->all(FLERR,"Read dump of MSUCG L property that isn't supported by atom style");
    if (type == Reader::MSUCGP && !atom->msucg_flag)
      error->all(FLERR,"Read dump of MSUCG P property that isn't supported by atom style");
    // UCG additions end
...
int ReadDump::whichtype(char *str)
{
  ...
  // UCG package
  else if (strcmp(str,"ucgstate") == 0) type = Reader::UCGSTATE;
  else if (strcmp(str,"ucgl") == 0) type = Reader::UCGL;
  else if (strcmp(str,"ucgp") == 0) type = Reader::UCGP;
  else if (strcmp(str,"msucgl") == 0) type = Reader::MSUCGL;
  else if (strcmp(str,"msucgp") == 0) type = Reader::MSUCGP;
  return type;
}
