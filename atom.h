...
class Atom : protected Pointers {
 public:
  ...
  int *ucgstate; // only allow integer values 0, 1, 2, ... (For now we only support 0 and 1 for two state systems)

  // For two-state systems
  double *ucgl, *ucgvl, *ucgml, *ucgp, *ucgforce; // Now we only allow 2-state systems, so ucgl is a 1D pointer.
  double **ucgsoftmaxscores;

  // UCGMS package
  int max_ucgstates;
  int *num_ucgstates;
  double **msucgl, **msucgth, **msucgvth, **msucgmth, **msucgp;
  double *msucgsoftmaxscores, **msucgforce, **msucgfth;
  //       Lambda,   Theta,     v_Theta,    m_Theta,    posterior,    f_Lambda,     f_Theta
  // Lambda is calculated from Theta, Theta is the real integrated variable here.
