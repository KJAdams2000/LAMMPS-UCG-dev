...
enum{ID,MOL,PROC,PROCP1,TYPE,TYPELABEL,ELEMENT,MASS,
     ...
     TQX,TQY,TQZ,
     UCGSTATE,UCGL,UCGP,MSUCGL,MSUCGP, // UCG package
...
int DumpCustom::count()
{
  ...
  if (nthresh) {
    ...
    for (int ithresh = 0; ithresh < nthresh; ithresh++) {
      ...
      // UCG additions
      else if (thresh_array[ithresh] == UCGSTATE) {
        if ((!atom->ucg_flag) && (!atom->msucg_flag))
          error->all(FLERR, Error::NOLASTLINE,
                     "Threshold for an atom property that isn't allocated");
        int *ucgstate = atom->ucgstate;
        for (i = 0; i < nlocal; i++) dchoose[i] = ucgstate[i];
        ptr = dchoose;
        nstride = 1;
      }
      else if (thresh_array[ithresh] == UCGL) {
        if (!atom->ucg_flag)
          error->all(FLERR, Error::NOLASTLINE,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->ucgl;
...
int DumpCustom::parse_fields(int narg, char **arg)
{
  ...
  for (int iarg = 0; iarg < narg; iarg++) {
    ...
    if (strcmp(arg[iarg],"id") == 0) {
      ...
      vtype[iarg] = Dump::DOUBLE;
      // UCG additions
    } else if (strcmp(arg[iarg],"ucgstate") == 0) {
      if (!atom->ucg_flag && !atom->msucg_flag)
        error->all(FLERR, errptr, "Dumping an atom property that isn't allocated");
      pack_choice[iarg] = &DumpCustom::pack_ucgstate;
      vtype[iarg] = Dump::INT;
    } else if (strcmp(arg[iarg],"ucgl") == 0) {
      if (!atom->ucg_flag)
        error->all(FLERR, errptr, "Dumping an atom property that isn't allocated");
      pack_choice[iarg] = &DumpCustom::pack_ucgl;
      vtype[iarg] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"ucgp") == 0) {
      if (!atom->ucg_flag)
        error->all(FLERR, errptr, "Dumping an atom property that isn't allocated");
      pack_choice[iarg] = &DumpCustom::pack_ucgp;
      vtype[iarg] = Dump::DOUBLE;
      // Multistate UCG additions, TODO
...
int DumpCustom::modify_param(int narg, char **arg)
{
  ...
  if (strcmp(arg[0],"thresh") == 0) {
    ...
    // UCG additions
    else if (strcmp(arg[1],"ucgstate") == 0) thresh_array[nthresh] = UCGSTATE;
    else if (strcmp(arg[1],"ucgl") == 0) thresh_array[nthresh] = UCGL;
    else if (strcmp(arg[1],"ucgp") == 0) thresh_array[nthresh] = UCGP;
    // Multistate UCG additions: will be introduced later
    // UCG additions end
...
void DumpCustom::pack_ucgstate(int n)
{
  int *ucgstate = atom->ucgstate;
  for (int i = 0; i < nchoose; i++) {
    buf[n] = ucgstate[clist[i]];
    n += size_one;
  }
}

void DumpCustom::pack_ucgl(int n)
{
...
void DumpCustom::pack_ucgp(int n)
{
