...
PairStyle(table_rleucg_interface,PairTable_RLEUCG_INTERFACE)

#else

#ifndef LMP_PAIR_TABLE_RLEUCG_INTERFACE_H
#define LMP_PAIR_TABLE_RLEUCG_INTERFACE_H

#include "pair.h"

namespace LAMMPS_NS {

class PairTable_RLEUCG_INTERFACE : public Pair {
 public:
  PairTable_RLEUCG_INTERFACE(class LAMMPS *);
  virtual ~PairTable_RLEUCG_INTERFACE();
 ...
 protected:
  ...
  int n_total_states;
  int n_actual_types;
  int max_states_per_type; // Can be used to generate the 
  int state_params_allocated;
  int *n_states_per_type;
  int *actual_types_from_state; // Map back from UCG state to actual type?
  ...
  // We can log the substate probabilities to a UCG atom style dump style.
  // This can be used to 
