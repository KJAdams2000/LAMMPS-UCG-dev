...
#include "pair_table_ucg_bethe.h"
...
static constexpr double EPSILONE = 1.0e-6;

PairTable_UCG_Bethe::PairTable_UCG_Bethe(LAMMPS *lmp) : Pair(lmp) 
{
    ntables = 0;
    nmax = 0;
    tables = nullptr;
    ...
    prior_prob_from_type = nullptr;
}

PairTable_UCG_Bethe::~PairTable_UCG_Bethe()
{
    if (copymode) return;
...
void PairTable_UCG_Bethe::allocate()
{
  allocated = 1;
...
void PairTable_UCG_Bethe::compute(int eflag, int vflag) {
    // from pair_table.cpp
    ...
    double pj0i0, pj1i0, pj0i1, pj1i1, pi0j0, pi1j0, pi0j1, pi1j1;
    ...
    // Initialize UCG softmax scores and per-atom num_ucgstates information with their chemical potentials.
    for (ii = 0; ii < inum; ii++) {
        i = ilist[ii];
        itype = type[i];
    ...
    // printf("(DEBUG) PairTable_UCG_Bethe::compute nlocal = %d, nall = %d, inum = %d\n", nlocal, nall, inum);
    ...
    for (ii = 0; ii < inum; ii++) {
        ...
        // Retrieve prior probabilities for atom i, either from previous ucgp[i] or from chemical potentials.

        if (ucgp[i] < -0.999) { // This scenario only works for the first evaluation of forces.
            if (n_states_per_type[itype] > 1) {
              if (prior_flag == CHEMICAL_POTENTIAL) {
                for (si = 0; si < n_states_per_type[itype]; si++) {
                    prior_prob[i][si] = prior_prob_from_type[itype][si];
                }
              }
              else if (prior_flag == CHEMICAL_POTENTIAL_NOISE) {
                randnum = (random->uniform() - 0.5) * 2;
                randnum *= noise_level;
                prior_prob[i][0] = std::min(0.999999, std::max(prior_prob_from_type[itype][0] + randnum, 0.0));
                prior_prob[i][1] = 1. - prior_prob[i][0];
              }
              else if (prior_flag == UCGL) {
                prior_prob[i][0] = 1.0 - ucgl[i];
                prior_prob[i][1] = ucgl[i];
              }
            }
            else prior_prob[i][0] = 1.0;              
        } 
        else {
            if (n_states_per_type[itype] > 1) {
              prior_prob[i][1] = ucgl[i];
        ...
        for (jj = 0; jj < jnum; jj++) {
            ...
            int procnum = (inum == 3); //debug info 
            // printf("(DEBUG) numneigh[i] = %d, i = %d, itype = %d, istate = %d, procnum = %d\n", jnum, i, itype, istate, procnum);

            // Retrieve prior probabilities for atom j, either from previous ucgp[j] or from chemical potentials.

            if (ucgp[j] < -0.999) {
                if (n_states_per_type[jtype] > 1) {
                    if (prior_flag == CHEMICAL_POTENTIAL) {
                      for (si = 0; si < n_states_per_type[itype]; si++) {
                          prior_prob[j][si] = prior_prob_from_type[itype][si];
                      }
                    }
                    else if (prior_flag == CHEMICAL_POTENTIAL_NOISE) {
                      randnum = (random->uniform() - 0.5) * 2;
                      randnum *= noise_level;
                      prior_prob[j][0] = std::min(0.999999, std::max(prior_prob_from_type[jtype][0] + randnum, 0.0));
                      prior_prob[j][1] = 1. - prior_prob[j][0];
                    }
                    else if (prior_flag == UCGL) {
                      prior_prob[j][0] = 1.0 - ucgl[j];
                      prior_prob[j][1] = ucgl[j];
                    }
                }
                else prior_prob[j][0] = 1.0;              
            } 
            else {
                if (n_states_per_type[jtype] > 1) {
                  prior_prob[j][1] = ucgp[j];
                  prior_prob[j][0] = 1.0 - ucgp[j];
                }
                else prior_prob[j][0] = 1.0;
            }
            ...
            if (rsq < cutsq[itype][jtype]) {
                ...
                // Scenario 2. i is a non-UCG atom, j is a UCG atom.
                    ...
                    fpair = prior_prob[j][0] * fpair00 + prior_prob[j][1] * fpair01;
                    evdwl = prior_prob[j][0] * u00 + prior_prob[j][1] * u01;
                }
                ...
                // Scenario 4/4. i and j are both UCG atoms.
                    ...
                    for (si = 0; si < n_states_per_type[itype]; si++) {
                        ...
                        for (sj = 0; sj < n_states_per_type[jtype]; sj++) {
                            ...
                            if (pseudo_flag == 0) {
                              // Pseudo-likelihood update happens inside this loop.
                              if (sj == jstate) {
                                  // Tally the contribution of neighbor j to the softmax score of i.
                    ...
                    Jij = u11 + u00 - u01 - u10;
                    if (Jij / kT < -709.0) {
                      error->warning(FLERR, "Jij/kT is too small for pair {} and {} : {} / {} < -709.0 at distance {}, Usisj = {}, {}, {}, {}", i, j, Jij, kT, std::sqrt(rsq), u00, u01, u10, u11);
                      Jij = -700.0 * kT; // Protect against numerical overflow
                    }

                    bij = std::exp(-Jij / kT);
                    aij = std::expm1(-Jij / kT);
                    ...
                    Qij = (pi1 + pj1) * aij + 1.;
                    Dij = Qij*Qij - 4. * aij * bij * pi1 * pj1;
                    Dij = std::max(Dij, 0.0);

                    // Protecting against numerical instability
                    
                    if (method_flag == BETHE) {
                      if (std::abs(aij) < EPSILONE) {
                        pij11 = pi1 * pj1;
                      } else {
                        if (Qij < 0.0) {
                          pij11 = (Qij - std::sqrt(Dij)) / (2. * aij);
                        } else {
                          pij11 = (2. * bij * pi1 * pj1) / (Qij + std::sqrt(Dij));
                        }
                      }
                    }
                    else if (method_flag == MF) {
                      pij11 = pi1 * pj1;
                    }
                    ...
                    if (pseudo_flag == 1) {
                      // If using full SCE to update posteriors, we tally the softmax scores here.
                      pj0i0 = pij00/pi0; // p(sj=0 | si=0)
                      pj0i1 = pij01/pi0;
                      pj1i0 = pij10/pi1;
                      pj1i1 = pij11/pi1;

                      pi0j0 = pij00/pj0; // p(si=0 | sj=0)
                      pi0j1 = pij10/pj0;
                      pi1j0 = pij01/pj1;
                      pi1j1 = pij11/pj1;

                      softmax_scores[i][0] -= (pj0i0 * u00 + pj1i0 * u01) / kT;
                      softmax_scores[i][1] -= (pj0i1 * u10 + pj1i1 * u11) / kT;
                      if (j < nlocal || newton_pair) {
                        softmax_scores[j][0] -= (pi0j0 * u00 + pi0j1 * u01) / kT;
                        softmax_scores[j][1] -= (pi1j0 * u10 + pi1j1 * u11) / kT;
                      }
                    }
    ...
    // We have moved the calculation of softmax scores to fix ucgstate
...
/* ----------------------------------------------------------------------
    ...
    5 7 2     --> 5 actual types, 7 formal types, maximum 2 states per type.
...
void PairTable_UCG_Bethe::read_state_settings(const char *file) {
    char *eof;
    char line[MAXLINE];
    ...
    // Read type and chemical potential information
    ...
    memory->grow(prior_prob_from_type, n_actual_types + 1, max_states_per_type, "pair:prior_prob_per_type");
    ...
    for (int i = 1; i <= n_actual_types; i++){
        ...
        if (n_states_per_type[i] == 2) {
            ...
            char *p2 = strtok(line, " ");
            for (int j = 0; j < n_states_per_type[i]; j++) {
                if (p2 == NULL) error->one(FLERR, "Not enough formal types specified for atom type %d.", i);
...
void PairTable_UCG_Bethe::settings(int narg, char **arg) {
    //setting the pseudo_flag of calculaing the softmax scores

    /* This initializes the pair style settings.
    Example:

    pair_style table_ucg_bethe linear  25000  ucg.conf  pseudo  no   pppm
               iarg              [0]    [1]      [2]     [3]    [4]  [5]...
            REQUIRED?             Y      Y        Y       N      N     N
    */
    pseudo_flag = 0; // default is pseudolikelihood method
    prior_flag = UCGL; // default is UCGL prior
    method_flag = BETHE;
    noise_level = 0.0;
    ...
    if (narg < 2) utils::missing_cmd_args(FLERR, "pair_style table_ucg_bethe", error);

    // Arg[0] designates the table style.
    ...
    // Arg[1] designates the table length.
    ...
    while (iarg < narg) {
        if (strcmp(arg[iarg], "ewald") == 0)
          ...
          iarg ++;
          if (iarg >= narg) error->all(FLERR, "Missing argument for pair_style table_ucg_bethe method");
          if (strcmp(arg[iarg], "mf") == 0 || strcmp(arg[iarg], "meanfield") == 0) {
            method_flag = MF;
          }
          else if (strcmp(arg[iarg], "bethe") == 0 || strcmp(arg[iarg], "Bethe") == 0) {
            method_flag = BETHE;
          }
          else {
            error->all(FLERR, "Unknown argument for pair_style table_ucg_bethe method: {}, please write mf or bethe", arg[iarg]);
          }
        }
        
        else if (strcmp(arg[iarg], "pseudo") == 0) {
          iarg++;
          if (iarg >= narg) error->all(FLERR, "Missing argument for pair_style table_ucg_bethe pseudo");
          if (strcmp(arg[iarg], "yes") == 0) {
            ...
            error->all(FLERR, "Unknown argument for pair_style table_ucg_bethe pseudo: {}, please write yes or no", arg[iarg]);
          }
        }
        else if (strcmp(arg[iarg], "prior") == 0) {
          iarg++;
          if (iarg >= narg) error->all(FLERR, "Missing argument for pair_style table_ucg_bethe");
          if (strcmp(arg[iarg], "chemical_potential") == 0) {
            iarg += 1;
            if (iarg >= narg) { // No additional arguments, just use chemical potential as prior
              prior_flag = CHEMICAL_POTENTIAL;
              iarg -= 1;
            } 
            else if (strcmp(arg[iarg], "noise") == 0) { // Add noise to chemical potential prior
              prior_flag = CHEMICAL_POTENTIAL_NOISE;
              iarg += 1;
              if (iarg >= narg) error->all(FLERR, "Missing argument for pair_style table_ucg_bethe prior chemical_potential noise: noise level must be set");
              noise_level = utils::numeric(FLERR, arg[iarg], false, lmp);
              if (noise_level <= 0.0) noise_level = 0.0;
              
              iarg += 1;
              if (iarg >= narg) error->all(FLERR, "Missing argument for pair_style table_ucg_bethe prior chemical_potential noise: random seed must be set");
          ...
          else if (strcmp(arg[iarg], "ucgl") == 0) {
            prior_flag = UCGL;
            // For EACH STEP, use ucgl for prior probabilities.
          }
          else {
            error->all(FLERR, "Unknown argument for pair_style table_ucg_bethe prior: {}, please write chemical_potential or ucgl", arg[iarg]);
          }
        }
        iarg++;
    }
...
void PairTable_UCG_Bethe::coeff(int narg, char **arg) {
   // Here, we require that all UCG interaction tables are provided in a single line of script.

   /* Pair_coeff command for pair_style table_ucg_bethe:
...
void PairTable_UCG_Bethe::init_style() {
    ...
    // after obtaining thermostat temperature, initialize the prior probabilities by type
    double denomi = 0.0;
    for (int i = 1; i <= n_actual_types; i++){
      if (n_states_per_type[i] == 0) continue;
      else if (n_states_per_type[i] == 1) {
        // For single state, we do not need to calculate the chemical potential.
        // We can just set it to zero.
        prior_prob_from_type[i][0] = 1.0;
      }
      else if (n_states_per_type[i] > 1) {
        for (int j = 0; j < n_states_per_type[i]; j++) {
          prior_prob_from_type[i][j] = std::exp(-chemical_potentials[formal_types_from_actual[i][j]] / kT);
          denomi += prior_prob_from_type[i][j];
        }
        for (int j = 0; j < n_states_per_type[i]; j++) {
          prior_prob_from_type[i][j] /= denomi;
        }
      denomi = 0.0;
    }
...
double PairTable_UCG_Bethe::init_one(int i, int j)
{
  if (setflag[i][j] == 0)
...
void PairTable_UCG_Bethe::read_table(Table *tb, char *file, char *keyword)
{
...
void PairTable_UCG_Bethe::bcast_table(Table *tb)
{
...
void PairTable_UCG_Bethe::spline_table(Table *tb)
{
...
void PairTable_UCG_Bethe::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTable_UCG_Bethe::compute_table(Table *tb)
{
  int tlm1 = tablength - 1;
...
void PairTable_UCG_Bethe::null_table(Table *tb)
{
...
void PairTable_UCG_Bethe::free_table(Table *tb)
{
...
void PairTable_UCG_Bethe::spline(double *x, double *y, int n, double yp1, double ypn, double *y2)
{
  int i, k;
  double p, qn, sig, un;
  auto u = new double[n];
...
double PairTable_UCG_Bethe::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo, khi, k;
  double h, b, a, y;
...
void PairTable_UCG_Bethe::write_restart(FILE *fp)
{
  write_restart_settings(fp);
}

void PairTable_UCG_Bethe::read_restart(FILE *fp)
{
  read_restart_settings(fp);
  allocate();
}

void PairTable_UCG_Bethe::write_restart_settings(FILE *fp)
{
...
void PairTable_UCG_Bethe::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTable_UCG_Bethe::single(int /*i*/, int /*j*/, int itype, int jtype, double rsq,
...
void *PairTable_UCG_Bethe::extract(const char *str, int &dim)
{
