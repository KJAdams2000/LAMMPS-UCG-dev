...
/* ----------------------------------------------------------------------
   Contributing author: Jaehyeok Jin and Alex Pak (UChicago)
------------------------------------------------------------------------- */
...
PairTable_RLEUCG_INTERFACE::PairTable_RLEUCG_INTERFACE(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
  // From PAIR_RLEUCG_INTERFACE_NEIGH
  nmax = 0;
...
PairTable_RLEUCG_INTERFACE::~PairTable_RLEUCG_INTERFACE()
{
...
void PairTable_RLEUCG_INTERFACE::threshold_prob_and_partial_from_cv(int type, double cv, double &prob, double &partial) 
{
  if (type == 1){
...
int PairTable_RLEUCG_INTERFACE::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairTable_RLEUCG_INTERFACE::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
int PairTable_RLEUCG_INTERFACE::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m,jsubstate;
...
void PairTable_RLEUCG_INTERFACE::unpack_forward_comm(int n, int first, double *buf)
{
...
// Compute local density (proximity function) using tanh functions
double PairTable_RLEUCG_INTERFACE::compute_proximity_function(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
  return 0.5 * (1.0 - tanh_factor);
}

double PairTable_RLEUCG_INTERFACE::compute_proximity_function_der(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
...
void PairTable_RLEUCG_INTERFACE::compute(int eflag, int vflag)
{
  ...
  evdwl = 0.0;
  ev_init(eflag, vflag); // Changes for 2024 LAMMPS
  ...
  if (nall > nmax) {
    nmax = nall;
    memory->grow(substate_probability, nall, max_states_per_type - 1, "pair/rleucg_interface:substate_probability");
    memory->grow(substate_probability_partial, nall, max_states_per_type - 1, "pair/rleucg_interface:substate_probability_partial");
    memory->grow(substate_probability_force, nall, max_states_per_type - 1, "pair/rleucg_interface:substate_probability_force");
    memory->grow(substate_cv_backforce, nall, 3, "pair/rleucg_interface:substate_cv_backforce");
  }
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    if (n_states_per_type[itype_actual] > 1) {
      ...
      // we use to measure state probability (currently, local density)
      for (jj = 0; jj < jnum; jj++) {
        j = jlist[jj];
        jtype = type[j];
        delx = xtmp - x[j][0];
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    if (n_states_per_type[itype_actual] > 1) {
      ...
      // Comment by XWZ: I don't think this is necessary? C++ doubles are not that fragile??
      if (use_state_entropy[itype_actual]) {
        for (isubstate = 0; isubstate < n_states_per_type[itype_actual] - 1; isubstate++) {
...
void PairTable_RLEUCG_INTERFACE::allocate()
{
  allocated = 1;
...
void PairTable_RLEUCG_INTERFACE::settings(int narg, char **arg)
{
...
void PairTable_RLEUCG_INTERFACE::read_state_settings(const char *file) {
  char *eof;
  char line[MAXLINE];
  ...
  // of actual types.
  // One additional PLACEHOLDER type 
...
void PairTable_RLEUCG_INTERFACE::coeff(int narg, char **arg)
{
...
void PairTable_RLEUCG_INTERFACE::init_style()
{
...
double PairTable_RLEUCG_INTERFACE::init_one(int i, int j)
{
...
------------------------------------------------------------------------- */
void PairTable_RLEUCG_INTERFACE::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];

  // open file

  FILE *fp = utils::open_potential(file, lmp, nullptr); // Changes for 2024 LAMMPS
  if (fp == nullptr) { // Changes for 2024 LAMMPS
    // char str[128]; // Changes for 2024 LAMMPS
    // sprintf(str,"Cannot open file %s",file); // Changes for 2024 LAMMPS
    error->one(FLERR,"Cannot open file %s", file, utils::getsyserror()); // Changes for 2024 LAMMPS
  }
...
void PairTable_RLEUCG_INTERFACE::bcast_table(Table *tb)
{
...
void PairTable_RLEUCG_INTERFACE::spline_table(Table *tb)
{
...
void PairTable_RLEUCG_INTERFACE::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTable_RLEUCG_INTERFACE::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairTable_RLEUCG_INTERFACE::null_table(Table *tb)
{
...
void PairTable_RLEUCG_INTERFACE::free_table(Table *tb)
{
...
void PairTable_RLEUCG_INTERFACE::spline(double *x, double *y, int n,
...
double PairTable_RLEUCG_INTERFACE::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTable_RLEUCG_INTERFACE::write_restart(FILE *fp)
{
...
void PairTable_RLEUCG_INTERFACE::read_restart(FILE *fp)
{
...
void PairTable_RLEUCG_INTERFACE::write_restart_settings(FILE *fp)
{
...
void PairTable_RLEUCG_INTERFACE::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTable_RLEUCG_INTERFACE::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairTable_RLEUCG_INTERFACE::extract(const char *str, int &dim)
{
