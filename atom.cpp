...
Atom::Atom(LAMMPS *_lmp) : Pointers(_lmp), atom_style(nullptr), avec(nullptr), avec_map(nullptr)
{
  ...
  ucgstate = nullptr;
  ucgl = ucgvl = ucgml = ucgp = nullptr;
  ucgforce = nullptr;
  ucgsoftmaxscores = nullptr;

  num_ucgstates = nullptr;
  max_ucgstates = 1; // 1 state means no UCG, 2 or more states means UCG
  msucgl = msucgth = msucgvth = msucgmth = msucgp = nullptr;
  msucgforce = msucgfth = nullptr;
...
void Atom::peratom_create()
{
  ...
  // UCG package (Lambda Dynamics only)

  add_peratom("ucgstate", &ucgstate, INT, 0);
  add_peratom("ucgl", &ucgl, DOUBLE, 0); // UCG Lambda Dynamics: Lambda Values; RLE-, Bethe-UCG: Posterior Probabilities
  add_peratom("ucgvl", &ucgvl, DOUBLE, 0); // UCG Lambda Dynamics: velocity of Lambda
  add_peratom("ucgml", &ucgml, DOUBLE, 0); // UCG Lambda Dynamics: pseudomass for Lambda
  add_peratom("ucgp", &ucgp, DOUBLE, 0); // Posterior Probabilities
  add_peratom("ucgforce", &ucgforce, DOUBLE, 0, 1); // UCG Lambda Dynamics: - dU / dLambda
  add_peratom("ucgsoftmaxscores", &ucgsoftmaxscores, DOUBLE, 2, 1); // for two state scenarios

  // Multistate UCG implementation
  add_peratom("num_ucgstates", &num_ucgstates, INT, 0);
  add_peratom("msucgth", &msucgth, DOUBLE, 1, 1);
  add_peratom("msucgl", &msucgl, DOUBLE, 1, 1);
  add_peratom("msucgvth", &msucgvth, DOUBLE, 1, 1);
  add_peratom("msucgmth", &msucgmth, DOUBLE, 1, 1);
  add_peratom("msucgp", &msucgp, DOUBLE, 1, 1);
  add_peratom("msucgforce", &msucgforce, DOUBLE, 1, 1);
  add_peratom("msucgfth", &msucgfth, DOUBLE, 1, 1);
  add_peratom("msucgsoftmaxscores", &msucgsoftmaxscores, DOUBLE, 1, 1);
...
void *Atom::extract(const char *name)
{
  ...
  if (strcmp(name, "ucgstate") == 0) return (void *) ucgstate;
  if (strcmp(name, "ucgl") == 0) return (void *) ucgl;
  if (strcmp(name, "ucgvl") == 0) return (void *) ucgvl;
  if (strcmp(name, "ucgml") == 0) return (void *) ucgml;
  if (strcmp(name, "ucgp") == 0) return (void *) ucgp;
  if (strcmp(name, "ucgforce") == 0) return (void *) ucgforce;
  if (strcmp(name, "ucgsoftmaxscores") == 0) return (void *) ucgsoftmaxscores;

  // Multistate UCG implementation

  if (strcmp(name, "num_ucgstates") == 0) return (void *) num_ucgstates;
  if (strcmp(name, "msucgl") == 0) return (void *) msucgl;
  if (strcmp(name, "msucgth") == 0) return (void *) msucgth;
  if (strcmp(name, "msucgvth") == 0) return (void *) msucgvth;
  if (strcmp(name, "msucgmth") == 0) return (void *) msucgmth;
  if (strcmp(name, "msucgp") == 0) return (void *) msucgp;
  if (strcmp(name, "msucgforce") == 0) return (void *) msucgforce;
  if (strcmp(name, "msucgfth") == 0) return (void *) msucgfth;
...
int Atom::extract_datatype(const char *name)
{
  ...
  if (strcmp(name, "ucgstate") == 0) return LAMMPS_INT;
  if (strcmp(name, "ucgl") == 0) return LAMMPS_DOUBLE;
  if (strcmp(name, "ucgvl") == 0) return LAMMPS_DOUBLE;
  if (strcmp(name, "ucgml") == 0) return LAMMPS_DOUBLE;
  if (strcmp(name, "ucgp") == 0) return LAMMPS_DOUBLE;
  if (strcmp(name, "ucgforce") == 0) return LAMMPS_DOUBLE;
  if (strcmp(name, "ucgsoftmaxscores") == 0) return LAMMPS_DOUBLE_2D;

  // Multistate UCG implementation
  
  if (strcmp(name, "num_ucgstates") == 0) return LAMMPS_INT;
  if (strcmp(name, "msucgl") == 0) return LAMMPS_DOUBLE_2D;
  if (strcmp(name, "msucgth") == 0) return LAMMPS_DOUBLE_2D;
  if (strcmp(name, "msucgvl") == 0) return LAMMPS_DOUBLE_2D;
  if (strcmp(name, "msucgml") == 0) return LAMMPS_DOUBLE_2D;
  if (strcmp(name, "msucgp") == 0) return LAMMPS_DOUBLE_2D;
  if (strcmp(name, "msucgforce") == 0) return LAMMPS_DOUBLE_2D;
  if (strcmp(name, "msucgfth") == 0) return LAMMPS_DOUBLE_2D;
...
int Atom::extract_size(const char *name, int type)
{
  ...
  if ((datatype == LAMMPS_DOUBLE_2D) || (datatype == LAMMPS_INT_2D)) {
    if (type == LMP_SIZE_ROWS) {
      ...
      if (strcmp(name, "ucgsoftmaxscores") == 0) return nall;

      // UCG Package - Multistate implementation

      if (strcmp(name, "msucgl") == 0) return nall;
      if (strcmp(name, "msucgth") == 0) return nall;
      if (strcmp(name, "msucgvth") == 0) return nall;
      if (strcmp(name, "msucgmth") == 0) return nall;
      if (strcmp(name, "msucgp") == 0) return nall;
      if (strcmp(name, "msucgforce") == 0) return nlocal;
      if (strcmp(name, "msucgfth") == 0) return nlocal;
      ...
      if (strcmp(name, "ucgsoftmaxscores") == 0) return atom->max_ucgstates;

      // UCG Package - Multistate implementation

      if (strcmp(name, "msucgl") == 0) return atom->max_ucgstates;
      if (strcmp(name, "msucgth") == 0) return atom->max_ucgstates;
      if (strcmp(name, "msucgvth") == 0) return atom->max_ucgstates;
      if (strcmp(name, "msucgmth") == 0) return atom->max_ucgstates;
      if (strcmp(name, "msucgp") == 0) return atom->max_ucgstates;
      if (strcmp(name, "msucgforce") == 0) return atom->max_ucgstates;
      if (strcmp(name, "msucgfth") == 0) return atom->max_ucgstates;
      if (strcmp(name, "msucgsoftmaxscores") == 0) return atom->max_ucgstates;
    ...
    if (strcmp(name, "ucgforce") == 0) return nall;
    if (strcmp(name, "ucgl") == 0) return nall;
    if (strcmp(name, "ucgp") == 0) return nall;
    if (strcmp(name, "ucgstate") == 0) return nall;
    if (strcmp(name, "ucgvl") == 0) return nall;
    if (strcmp(name, "ucgml") == 0) return nall;

    // Multistate UCG implementation

    if (strcmp(name, "num_ucgstates") == 0) return max_ucgstates;
